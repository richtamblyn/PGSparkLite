...
class SparkAmpServer:    
    ...
    def __init__(self, socketio):        
        ...
        self.notifier = Notifier(["callback","connection_lost","preset_corrupt"])
    ...
    def callback_event(self, data):        
        ...
        if 'PresetNumber' in data:
            if self.config == None or self.config.preset != data['PresetNumber']:            
        ...
        if 'Effect' in data:
            ...
            self.config.update_config(effect, 'change_parameter', value, parameter)
            ...
            self.socketio.emit('update-onoff', {'effect': effect, 'state': state})
    ...
    def connect(self):        
        try:
            ...
            self.socketio.emit('connection-message', {'message': 'Retrieving Amp configuration...'})
            self.initialise()

        except:            
            self.socketio.emit('connection-message', {'message': 'Connection failed.'})    
